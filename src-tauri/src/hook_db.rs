...
pub(crate) fn resolve_db_path(app: &tauri::AppHandle, db_path: Option<String>) -> Result<PathBuf, String> {
    let path = db_path.map(|p| p.trim().to_string()).filter(|p| !p.is_empty())
        .map(PathBuf::from).unwrap_or(app.path().home_dir().map_err(|e| format!("home_dir_unavailable: {e}"))?.join(".cc-switch").join("cc-switch.db"));
    if path.is_file() { Ok(path) } else { Err("db_not_found".to_string()) }
}
